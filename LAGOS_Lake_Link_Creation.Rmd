...
Analysis on the early versions of LAGOS Lake Link showed that a signification fraction (about 25%) of lakes that should be found both the NHDPlusv2 and the NHD HR could not be linked through the reachcodes or names. As a result, we will add linkages for these lakes through the spatial relationships.

Some thoughts:
I don't want to add links based on spurious overlaps of the edges of lakes with nearby waterbodies.
So try the centroids first, but this method has shortcomings.
Convert the larger feature set to centroids to keep it fast and simple.
```{r}

lagos_nhd_not_missing <- lagos_nhd_all %>%
  filter(!is.na(COMID))

lagos_nhd_is_missing <- lagos_centroid %>%
  inner_join((
  lagos_nhd_all %>%
  anti_join(lagos_nhd_not_missing, by = "lagoslakeid") %>%
  left_join(nhd_gnis_nhd, by = "lagoslakeid") %>%
  distinct(lagoslakeid, GNIS_Name)
  ),
  by = "lagoslakeid")

test <- nhd_plus_orig %>%
  st_join(lagos_nhd_is_missing)
...
```{r write-final}
#write_csv(final, 'LAGOS_Lake_Link_v0.csv')
```
