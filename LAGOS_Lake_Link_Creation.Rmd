...
lagos <- lagos_sf %>%
  select(lagoslakeid, 
         NHDHR_PermanentIdentifier = Permanent_Identifier, 
         GNIS_ID, 
         GNIS_Name, 
         NHDHR_ReachCode = ReachCode, 
         LAGOS_State = STATE, 
         NHDHR_AreaSqKm = AreaSqKm, 
         NHDHR_FDate = FDate,
         LAGOS_CountyName = NAMELSAD,
         LAGOS_CountyFIPS = FIPS)

lagos_centroid <- lagos %>%
  st_centroid() %>%
  st_transform(NAD83) %>%
  mutate(LAGOS_LakeLatitude = st_coordinates(.)[,'Y'], 
         LAGOS_LakeLongitude = st_coordinates(.)[,'X']) %>%
  select(lagoslakeid, NHDHR_PermanentIdentifier, NHDHR_ReachCode, LAGOS_State, NHDHR_AreaSqKm, NHDHR_FDate, LAGOS_LakeLatitude, LAGOS_LakeLongitude, LAGOS_CountyName, LAGOS_CountyFIPS) %>%
...
library(tidyverse)
results <- gnis_orig %>%
  filter(grepl("aspen", FEATURE_NAME, ignore.case = TRUE))
View(results)
?grepl
```
...
lagos_gnis <- lagos_gnis_3 %>%
  st_set_geometry(NULL) %>%
  select(lagoslakeid, GNIS_ID, GNIS_Name, LAGOS_LakeName)
```
...
Examine the distribution of areal overlaps. It seems like 
```{r}
...
## Select again
```{r}
lagos_nhd_spatial <- nhd_lagos_spatial_accept %>%
  ...
  mutate(NHDPlusv2_AreaSqKm = AREASQKM) %>%
  select(lagoslakeid, NHDPlusv2_COMID, NHDPlusv2_ReachCode, NHDPlusv2_GNIS_Name, NHDPlusv2_AreaSqKm)
```
...
lagosUS_lagosNE <- lagos_lagosNE_all %>%
  ...
  arrange(lagoslakeid) %>%
  left_join(legacy_ids, by = "lagoslakeid") %>%
  select(lagoslakeid,
         LAGOSNE_lagoslakeid, 
         LAGOSNE_legacyid = legacyid)
...
together <- lagos_centroid %>%
  left_join(lagos_gnis, by = "lagoslakeid") %>%
  left_join(lagos_wqp, by = "lagoslakeid") %>%
  left_join(union(lagos_nhd, lagos_nhd_spatial), by = "lagoslakeid") %>%
  left_join(lagosUS_lagosNE, by = "lagoslakeid") %>%  
  left_join(nla2012_lagos, by = "lagoslakeid")

counts_round1 <- together %>%
  group_by(lagoslakeid) %>%
  summarize(count_wqp_per_lagos_id = n_distinct(WQP_MonitoringLocationIdentifier, na.rm = TRUE), 
...
distinct(final, count_lagos_per_nhdplusv2_id)
```
...
```{r write-final}
write_csv(final, 'LAGOS_Lake_Link_v1_20190128.csv')
```
...
E) Manual links for any NLA sites that still don't connect if we add the spatial analysis in TO-DO item (B).
F) Merge NHDPlusV2 names into the semi-colon list (cut for time--needs another join/distinct/verbose name calculation chunk)
...
G) A user cannot find identifiers pertaining to a lake that isn't in LAGOS-US. SOLUTION: Use outer joins and allow the crosswalk to grow a fair amount in size to allow NULLs in any of the identifiers. Revise the documentation to reflect the expansion.
H) A user cannot connect WQP sites to the NHDPlusV2 representation of lakes. Example: 2 NHDPlusV2 lakes to 1 LAGOS-US lake. SOLUTION: Refine the entity-relationship model to join between the WQP and NHDPlusV2 directly. Rework subsequent joins in the script to accomodate the change. This change probably won't affect the overall size or usability of the crosswalk too much.

## Proposed ER Diagram demonstrating change "H" above
![*Proposed entity-relationship diagram showing two additions, in color*](./images/ERD_proposed.PNG)
